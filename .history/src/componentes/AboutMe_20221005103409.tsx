...
export const AboutMe = () => {
	...
	return (
		<VStack w={"full"} bg={"black"}>
			<VStack w={"full"}>
				<VStack alignItems={"start"} w={"full"} h={"100vh"} backgroundImage={bg} className={"img-bg"}>
					<VStack mt={10} ml={10} w={"100px"} h={"60px"} >
						<Text fontFamily={"Kaushan Script"} fontSize={"36px"} color={"white"}>Thislucasmed</Text>
					</VStack>
								...
								<Center w={"full"}>
									{!isLargerThan1016 ?
										<Text className={"escrevendo"} style={{ fontSize: "100px", fontFamily: "Poppins", lineHeight: "46px", fontWeight: "600px", letterSpacing: "0.02em" }} fontWeight={"semibold"} color={"white"}>
											<Typewriter
												words={['Desenvolvedor', 'Estudante', 'Sigma']}
												loop={5}
												cursor
												cursorStyle='_'
												typeSpeed={70}
				...
				<VStack w={"full"} bg={"black"}>
					<VStack mt={10}>
						<Text fontFamily={"Kaushan Script"} fontSize={"36px"} letterSpacing={4} color={"white"}>Últimos projetos</Text>
					</VStack>
					<SimpleGrid w={"90%"} py={10} columns={[1, null, 3]} spacing='40px'>
						<Box bg='gray.900' borderRadius={4} p={4}>
							<Image borderRadius={4} src={creepyteller} />
							<br />
							<VStack spacing={5}>
